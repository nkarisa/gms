...
class LanguageLibrary extends GrantsLibrary implements \App\Interfaces\LibraryInterface
{
  ...
  function getLanguages()
  {
   ...
   /**
   * actionAfterInsert(): This method overides the instial method in grantsLibrary to action an update on language record.
   * @author  Livingstone Onduso
   * @dated: 13/05/2023
   * @access public
   * @return bool
   * @param array $post_array, int $approval_id, int $header_id
   */
  function actionAfterInsert($post_array, $approval_id, $header_id): bool
  {
    $this->write_db->transStart();
    //Get the language code after save
   $builder=$this->write_db->table('language');
   $builder->where(['language_id'=>$header_id]);
   $languageCode=$builder->get()->getRow()->language_code;

   //Upate code after save
   $data['language_code']=strtolower($languageCode);
   $updateLanguageCode=$this->write_db->table('language');
   $updateLanguageCode->where(['language_id'=>$header_id]);
   $updateLanguageCode->update($data);
