...
class MessageLibrary extends GrantsLibrary implements \App\Interfaces\LibraryInterface
{
  ...
  function notesHistory(int $item_id)
  {
    ...
    $builder->select(array(
      'user_id',
      'message_detail_last_modified_by as last_modified_by',
      "message_id",
      'message_detail_last_modified_date as last_modified_date',
      'message_record_key',
      "message_detail_id",
      "CONCAT(user_firstname, ' ', user_lastname) as creator",
      'message_detail_content as body',
      'message_detail_created_date as created_date',
      'message_detail_readers as message_readers'
    ));
  ...
  function markNoteAsRead($reader_user_id, $message_detail_id)
  {

    $readers = "[]";

    //Get the name
    $reader_builder = $this->read_db->table('user');
    $reader_builder->select("CONCAT(user_firstname, ' ', user_lastname) as fullname");
    $reader_builder->where('user_id', $reader_user_id);
    $reader_user_fullname = $reader_builder->get()->getRow()->fullname;


    $reader_builder_msg = $this->read_db->table('message_detail');
    $reader_builder_msg->select('message_detail_readers');
    $reader_builder_msg->where('message_detail_id', $message_detail_id);
    $query_msg = $reader_builder_msg->get();
    $message_detail_readers_array = $query_msg->getRowArray();
    ...
    $builder = $this->write_db->table('message_detail');
    $builder->where('message_detail_id', $message_detail_id);
    $builder->update($data);
  }
