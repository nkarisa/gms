...
class ExpenseVoteHeadsCategoryLibrary extends GrantsLibrary implements \App\Interfaces\LibraryInterface
{
    ...
    public function checkAndCreateDepreciationExpenseVoteHeadCategory(){
        ...
        $expenseVoteHeadsCategoryReadBuilder->where(['expense_vote_heads_category_name' => 'Depreciation']);
        $expenseVoteHeadsCategoryObj = $expenseVoteHeadsCategoryReadBuilder->get();

        $depreciationExpenseVoteHeadsCategoryId = 0;
        // Create the vote head if not present
        if($expenseVoteHeadsCategoryObj->getNumRows() > 0){
            $depreciationExpenseVoteHeadsCategoryId = $expenseVoteHeadsCategoryObj->getRowArray()['expense_vote_heads_category_id'];
        }else{
            ...
            $voteHeadData['expense_vote_heads_category_name'] = 'Depreciation';
            $voteHeadData['expense_vote_heads_category_description'] = 'Depreciation';
            ...
            $depreciationExpenseVoteHeadsCategoryId = $this->write_db->insertID();
        }

        return $depreciationExpenseVoteHeadsCategoryId;
    }
