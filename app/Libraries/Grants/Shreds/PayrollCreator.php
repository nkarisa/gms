...
class PayrollCreator implements \App\Libraries\Grants\Shreds\SchedulerGenerator {
    ...
    private function createPayroll($officeId, $officeAccountSystemId, $reportingMonth)
    {
        ...
        $nameAndTrackNumber = $grantsLibrary->generateItemTrackNumberAndName('payroll');
        ...
        $newPayroll = [
            ...
            'payroll_created_by' => $this->session->user_id ?? 1,
            'payroll_last_modified_by' => $this->session->user_id ?? 1,
            'fk_status_id' => $initialStatusId
        ];

        $postData = new PostData($newPayroll, 'payroll', false);

        return $grantsLibrary->add($postData);
    }

    private function getExistingPayrollId($officeId, $reportingMonth){
      ...
      private function officeUserBuilder($officeId){
     ...
     private function getActiveOfficeStaffWithPayHistory($officeId, $officeAccountSystemId){
        ...
        $userBuilder = $this->officeUserBuilder($officeId);
    ...
    private function prepareUsersPayslipInfo($officeUsers, $payrollId)
    {
        ...
        foreach ($officeUsers as $officeUser) {
            $nameAndTrackNumber = $grantsLibrary->generateItemTrackNumberAndName('payslip');
            ...
            $payslipData[$cnt] = [
                ...
                'payslip_created_by' => $this->session->user_id ?? 1,
                'payslip_last_modified_by' => $this->session->user_id ?? 1
            ];
    ...
    public function getActiveTransactingOffices(): array{
        ...
        if(count($accountSystemIds) > 0){
            $officeBuilder->whereIn('office.fk_account_system_id', [3]);
        }
    ...
    {   
        ...
        $payrollId = !$payrollResult['flag'] ?
            $this->getExistingPayrollId($officeId, $reportingMonth) :
            hash_id($payrollResult['header_id'],'decode');
