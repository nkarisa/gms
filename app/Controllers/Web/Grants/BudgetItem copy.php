...
class BudgetItem extends WebController
{
    ...
    public function updateBudgetItem($budget_item_id)
    {
        ...
        foreach ($this->request->getPost('fk_month_id') as $month_id => $month_amount) {
            if ($month_amount > 0) {
                ...
                $row[] = $this->createUpdateArrayForJson($budget_item_id, $month_amount, $month_id);
            }
        }
        
        /*
        This code added by Onduso to resolve the sign off budget issue: (users not able to sign off budget) on 22/04/2025
        Rally Defect:DE4493 :Budget sign-off issue: FCP not able to signoff the budgets.
        */

        //Check if $row is empty when editing.

        if (empty($row)) {

            $row[] = $this->createUpdateArrayForJson($budget_item_id);

            $header['budget_item_unit_cost'] = 0.00;
        }
    ...
    /**
     * create_update_array_for_json: this method return as array for updating the budget item.
     * @param  $budget_item_id,float $amount=0.00,int $month=1
     * @access private
     * @return array
     * @author Livingstone Onduso
     * @date 22/04/2025
     */
    private function createUpdateArrayForJson($budget_item_id, float $amount = 0.00, int $month = 1): array
    {
        ...
        $data['budget_item_detail_amount'] = $amount;
        $data['fk_month_id'] = $month;
