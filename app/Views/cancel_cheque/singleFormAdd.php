    ...
    <div class='col-xs-12'>
        <div class="panel panel-default" data-collapsed="0">
            ...
            <div class="panel-body" style="max-width:50; overflow: auto;">
                ...
                <div class='form-group'>
                    ...
                    <label
                        class='col-xs-2 control-label'><?= get_phrase('valid_cheques', "Valid Cheque Numbers") ?></label>
                    <div class='col-xs-2'>

                        <select class="form-control select2 required" id="cheque_number_id" name='cheque_number_id[]'
                            multiple>
                        </select>
                    </div>
                    ...
                    <div class='col-xs-2'>
                        <select class="form-control select2 required" id="fk_item_reason_id" name='fk_item_reason_id'>
                            ...
                            foreach ($reason_ids_and_names as $key => $reason_ids_and_name) { ?>
                                <option value=<?= $key ?>> <?= get_phrase('item_reason_' . $key, $reason_ids_and_name) ?>
                                </option>
                                <?php
                            }
                            ?>
                ...
                <div class='form-group'>
                    <label class='col-xs-2 control-label hidden'
                        id='other_reason_label_id'><?= get_phrase('other_reason_field', "Other Reason Details") ?></label>
                    <div class='col-xs-10'>
                        <input class="form-control hidden col-xs-10" id="other_reason" name='other_reason'
                            type='text' />
                    </div>
                </div>
                ...
                <div class='form-group'>
                    <div class='col-xs-12' style='text-align:center;'>
                        <button
                            class='btn btn-default btn-save'><?= get_phrase("cancel_cheque_no", "Cancel Cheque Number(s)") ?></button>
        ...
        <script>
            ...
            $(document).on('change', '#fk_office_bank_id', function () {
                ...
                let active_cheque_book_url = '<?= base_url() ?>ajax/cancel_cheque/getActiveChequeBook';
                let data = {
                    office_bank_id
                }

                $.post(active_cheque_book_url, data, function (response) {
                    console.log(response)
                    let res = parseInt(response.active_cheque_books);
                    if (res == 0) {
                    ...
                    let url = '<?= base_url() ?>ajax/cancel_cheque/getValidCheques/';
                    let data = {
                        office_bank_id
                    }
                    ...
                    $.post(url, data, function (response) {
                        ...
                        const cheque_numbers = response.cheque_numbers
