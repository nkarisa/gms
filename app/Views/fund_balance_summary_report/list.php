...
<div id = "warning_holder" class="row <?=!session()->get('system_admin') ? 'hidden_elem' : '';?>">
    <div class="well col-xs-12" style="text-align:center;font-weight:bold;color:red;"><?=get_phrase('national_office_not_selected',"National Office Not Selected");?></div>
</div>
...
<div id = "month_label_holder" class = "row <?=session()->get('system_admin') ? 'hidden_elem' : '';?>">
    <div id="balance_label" class="col-xs-12" style="text-align: center;margin-top:20px;font-weight:bold;">
        <?=get_phrase('monthly_balances_for_ending_period',"Monthly Balances for the Period Ending")?> <span id = "period"><?=$month;?></span>
    </div>

	<div id="income_label" class="col-xs-12 d-none" style="text-align: center;margin-top:20px;font-weight:bold;">
		<?=get_phrase('monthly_income_report_for_the_period',"Monthly Income Report for the Period")?> <span class="month_period" id = "period"></span>
	</div>

	<div id="expense_label" class="col-xs-12 d-none" style="text-align: center;margin-top:20px;font-weight:bold;">
		<?=get_phrase('monthly_expense_report_for_the_period',"Monthly Expense Report for the Period")?> <span class="month_period" id = "period"></span>
	</div>
	...
	$(document).ready(function () {
		...
		$(document).ready(function() {
			...
			function isRangeValid() {
				...
				var oneYearFromStart = new Date(startDate.getFullYear() + 1, startDate.getMonth(), startDate.getDate());
			...
			function checkAndTriggerInit() {
				if (isRangeValid()) {
					...
					alert('<?=get_phrase('invalid_date_range',"Invalid date range");?>')
				}
			}
			...
			$('#start_date').datepicker({
				...
				minViewMode: 1,
				autoclose: true,
				startDate: new Date(today.getFullYear() - 1, today.getMonth(), 1), // 1 year in the past
				...
				// Limit the end date to either one year from the start date or maxEndDate, whichever is earlier
				var newMaxEndDate = (oneYearFromStart < maxEndDate) ? oneYearFromStart : maxEndDate;

				// Update the end datepicker's min and max values
				$('#end_date').datepicker('setStartDate', startDate);
				$('#end_date').datepicker('setEndDate', newMaxEndDate);

				// If the current end date exceeds the new maximum, reset it
				var currentEndDate = $('#end_date').datepicker('getDate');
				if (currentEndDate > newMaxEndDate) {
					$('#end_date').datepicker('setDate', newMaxEndDate);
				}
			...
			$('#end_date').datepicker({
				...
				// Prevent the end date from being earlier than the selected start date
				var startDate = $('#start_date').datepicker('getDate');
				if (endDate < startDate) {
					$('#end_date').datepicker('setDate', startDate);
				}
